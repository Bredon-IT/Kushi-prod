  ...
  // Handle click outside dropdown
   useEffect(() => {
    const handleClickOutside = (event: MouseEvent) => {
      ...
      setIsInputFocused(false); // Also reset the focus state
           }
         };
...
// Helper Component for Portal (DEFINED INSIDE HomePage)
const SearchDropdownPortal: React.FC = () => {
    // We need to calculate where to place the dropdown relative to the search input.
    const [rect, setRect] = useState<DOMRect | null>(null);

    // Calculate position when search term changes or dropdown opens/closes
    useEffect(() => {
        if (showDropdown && searchRef.current) {
            // Get the bounding box of the search input wrapper div (relative to viewport)
            const searchRect = searchRef.current.getBoundingClientRect();
            
            // Set position data
            setRect({
                left: searchRect.left,
                top: searchRect.bottom, // Use the bottom of the input wrapper (viewport Y coordinate)
                width: searchRect.width,
                height: 0, // Not strictly needed
                x: searchRect.x,
                y: searchRect.y,
...
    
    // Only render if the dropdown should be visible AND we have the position data AND we have suggestions
    if (!showDropdown || !rect || suggestions.length === 0) return null;

    // Use a high z-index (e.g., z-[9999]) to ensure it's on top of everything
    const portalStyle: React.CSSProperties = {
        position: 'absolute',
        // Crucial Fix: rect.top is the bottom of the input relative to viewport. Add scrollY for document position + 8px gap.
        top: rect.top + window.scrollY + 8, // 8px margin below input
        left: rect.left + window.scrollX,
        width: rect.width, // Match the width of the search wrapper
...
                            // Use onMouseDown instead of onClick to prevent 'onBlur' from firing before click
...
    // Render the dropdown into the document body
...
<section className="relative min-h-[2vh] flex flex-col justify-center overflow-hidden bg-gradient-to-r from-peach-300 to-navy-700">
  ...
  <div className="relative w-full max-w-7xl mx-auto px-6 lg:px-12 py-8 text-left z-10 flex flex-col justify-center items-start">
   ...
   {/* --- Search Bar (FIXED Alignment and Dropdown) --- */}
<div className="flex flex-col md:flex-row items-start md:items-center gap-4 w-full">

    
    {/* Search Wrapper - HIGH Z-INDEX APPLIED HERE */}
     <div className="max-w-xs w-full relative order-1 md:order-1 z-50" ref={searchRef}>
...
              
             // --- INLINE STYLE OVERRIDE (Strikethrough Fix) ---
              style={{
                textDecoration: isInputFocused ? 'none' : 'initial',
...
              }}
              onBlur={() => {
                // Keep a small delay to allow click on suggestion before closing
                setTimeout(() => {
                  setShowDropdown(false); // Hide the dropdown on blur
                  setIsInputFocused(false); 
...
        {/* NOTE: The inline dropdown JSX previously here has been REMOVED. 
             It is now rendered via the <SearchDropdownPortal /> component below. */}
        
