...
const Blog: React.FC = () => {
  ...
  // We will map the posts twice later to enable the loop
  const postsToDisplay = [...filteredPosts, ...filteredPosts]; 
  ...
  return (
    <div className="bg-white">
      ...
      {/* Blog Content - Horizontal Auto-Scrolling Carousel */}
      <section className="py-6 bg-white overflow-hidden">
                ...
                <button
                  ...
                  className={`px-3 py-1.5 rounded-full text-sm font-medium transition-all ${
                    selectedCategory === category
                      ? 'bg-gradient-to-r from-peach-300 to-navy-700 text-white shadow-md'
                      : 'bg-orange-100 text-peach-600 hover:bg-navy-700 hover:text-white'
                  }`}
                >
                  {category}
                </button>
        ...
        {/* Only display the carousel if there are posts to show */}
        {filteredPosts.length > 0 && (
          <div 
            className="flex space-x-6 px-4 pb-4 blog-scroller-container"
            style={{ scrollbarWidth: 'none', width: 'fit-content' }}
          >
            
            {/* Map over the duplicated posts array */}
            {postsToDisplay.map((post, index) => (
              <article
                key={`${post.id}-${index}`} // Use index in key to distinguish duplicates
                className="flex-shrink-0 w-80 bg-white rounded-xl overflow-hidden shadow-lg border border-orange-200 hover:shadow-xl transition-all transform hover:scale-[1.01] max-w-sm"
              >
                ...
                <div className="p-4">
                  ...
                  <div className="flex justify-start">
                    <Link
                      to={`/blog/${post.id}`}
                      className="inline-flex items-center gap-2 px-3 py-1.5 bg-peach-300 text-white rounded-md text-sm font-medium hover:bg-peach-400 transition-colors"
                    >
                      Read More
                      <ArrowRight size={14} />
                    </Link>
      ...
      <style jsx global>{`
        ...
        .blog-scroller-container {
          /* Animation speed: Adjusted for blog posts */
