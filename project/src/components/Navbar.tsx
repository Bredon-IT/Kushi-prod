...
const Navbar: React.FC<{ cartItemCount?: number }> = ({ cartItemCount = 0 }) => {
  ...
  const locations = [{ name: "Bangalore", image: "/bangalore.png" }, { name: "Hyderabad", image: "/Hyderabad.png" }];
  const navItems = [{ name: "Home", path: "/" }, { name: "Services", path: "/services" }];
  const menuItems = [{ name: "Gallery", path: "/gallery" }, { name: "Blog", path: "/blog" }, { name: "About Us", path: "/about" }, { name: "Contact Us", path: "/contact" }];
  const isActive = (path: string) => location.pathname === path;
  const handleLogout = () => { logout(); setIsUserMenuOpen(false); setIsMenuOpen(false); navigate("/signin"); };
  const handleLocationSelect = (city: string) => { setLocation(city); setIsLocationOpen(false); };
  ...
  return (
    ...
    ${isScrolled ? "bg-sky-200/95 shadow-xl" : "bg-sky-100/95"}`}>
      <div 
      className="max-w-10xl mx-auto px-4 sm:px-6 lg:px-8 flex items-center justify-between h-20">
        <Link 
        to="/" 
        className="flex-shrink-0 flex items-center">
          <img src="/kushilogo.jpg" 
          alt="Logo" 
          ...
          {navItems.map((item) => (
            <Link
             ...
             className={`px-3 py-2 rounded-md text-sm font-semibold transition-colors duration-200
               ${isActive(item.path) ? "text-peach-300 bg-white shadow-sm" : "text-navy-700 hover:text-peach-500 hover:bg-sky-200"}`}>
                {item.name}
                </Link>
          ))}
          ...
          <div className="relative" ref={userDropdownRef}>
            ...
            {isUserMenuOpen && (
              <div className="absolute right-0 mt-2 w-56 bg-white shadow-xl rounded-lg border border-gray-100 py-2 z-50">
                {menuItems.map((item) => (
                  <Link
                   key={item.name} to={item.path!} onClick={() =>
                    setIsUserMenuOpen(false)} 
                    className="block px-4 py-2 text-sm text-navy-700 hover:bg-sky-100 transition-colors duration-200">
                      {item.name}
                      </Link>
                    ))}
        ...
        <div className="lg:hidden flex items-center">
          ...
          className="relative p-2 rounded-full text-navy-700 hover:bg-peach-200 mr-2 transition-colors duration-200">
            ...
            {cartCount > 0 &&
             <span className="absolute top-0 right-0 bg-red-500 text-white text-xs rounded-full h-4 w-4 flex items-center justify-center font-bold">{cartCount}
             </span>}
          </Link>
          ...
          <div ref={mobilePanelRef}
           className="absolute top-0 right-0 w-64 h-full bg-white shadow-2xl p-4 flex flex-col overflow-y-auto">
            <div className="flex justify-end mb-4">
              ...
              className="p-2 rounded-md text-navy-700 hover:bg-peach-300"><X size={22} />
              </button>
            </div>
            <Link 
            to="/booking" 
            ...
            <div className="mb-4 p-2 bg-sky-50 rounded-lg">
              <p className="text-sm font-bold mb-2 text-navy-700">
                Current Location: {selectedLocation || "Not Set"}</p>
              <hr className="my-2 border-gray-200" />
                 ...
                 (<button key={city.name}
                  onClick={() => { handleLocationSelect(city.name);
                   setIsMenuOpen(false); 
                    ...
                    className="flex items-center gap-2 w-full text-left px-3 py-2 text-sm text-navy-700 hover:bg-sky-100 rounded transition-colors duration-200">
                      <img src={city.image}
                       alt={city.name} 
                       className="h-6 w-10 object-cover rounded shadow-inner" />
                       <span>
