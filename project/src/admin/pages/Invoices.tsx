...
export const updateInvoicePaymentStatus = async (id: number | string, status: string) => {
...
export function Invoices() {
  ...
  const [selectedInvoices, setSelectedInvoices] = useState<(number | string)[]>([]);

  // Helper to determine payment status: Check 'payment_status' first, then worker_assign as a fallback for 'unpaid'
  const getInvoicePaymentStatus = (inv: Invoice) => {
    ...
    return (Array.isArray(inv.worker_assign) && inv.worker_assign.length > 0) ? "paid" : "unpaid";
  };
  ...
  const toggleInvoiceSelection = (id: number | string) => {
  ...
  const handlePaymentStatusChange = async (id: number | string, newStatus: string) => {
    try {
                    ...
                    <tbody className="bg-white divide-y divide-gray-100 dark:bg-gray-800 dark:divide-gray-700">
                      {filteredInvoices.map((inv) => {
                        ...
                        return (
                          <tr
                            key={inv.booking_id}
                            className={`hover:bg-gray-50 dark:hover:bg-gray-700/50 transition-colors ${selectedInvoices.includes(inv.booking_id)
