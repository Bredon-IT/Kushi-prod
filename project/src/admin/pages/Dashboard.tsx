...
interface DashboardBooking {
  ...
  // Raw API properties if needed for filtering
...
export function Dashboard() {
  ...
  const [topServices, setTopServices] = useState<any[]>([]); // Assuming same as TopRatedService or simple list
  const [topBookedCustomers, setTopBookedCustomers] = useState<TopBookedCustomer[]>([]);
  ...
  useEffect(() => {
    const fetchRecentBookings = async () => {
      try {
        ...
        const mapped: DashboardBooking[] = res.data.map((item: any) => ({
          id: item.booking_id,
          ...
          // Retain original fields if needed for other logic
  ...
  useEffect(() => {
    ...
    axios.get(Global_API_BASE + "/api/admin/all-bookings")
      .then(res => {
        ...
        const todaysList = allBookings.filter((b: any) => {
  ...
  useEffect(() => {
    const fetchServiceReportCsv = async () => {
      try {
        ...
        console.warn("Service report CSV fetch failed (expected in local env if backend missing):", err.message);
      }
    };
  ...
  useEffect(() => {
    OverviewService.getDashboardOverview()
      ...
      .catch((err: any) => console.error('Error fetching dashboard overview:', err));
  }, []);
    ...
    <CardContent className="p-3 md:p-4">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 md:gap-6">
        ...
        <Card className="shadow-lg border-0 overflow-hidden">
          ...
          <CardContent className="p-3 md:p-4">
            <div className="space-y-3">
              {topBookedCustomers.slice(0, 6).map((customer, index) => (
                <div
                ...
                <tbody>
                  {currentBookings.map((booking, index) => (
                    <tr
                      ...
                      className={`border-b border-gray-100 dark:border-gray-700 hover:bg-gradient-to-r hover:from-gray-50 hover:to-gray-100 dark:hover:from-gray-800 dark:hover:to-gray-700 transition-all ${index % 2 === 0 ? "bg-gray-50/50 dark:bg-gray-800/50" : ""
                        }`}
                    >
                      {/* Customer */}
