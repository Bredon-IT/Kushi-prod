...
const removePackage = (index: number) => setPackages(packages.filter((_, i) => i !== index));
  ...
  const handleFormSubmit = async (e: React.FormEvent) => {
    ...
    // send all text fields as JSON string
    ...
    if (serviceImageFile) {
      ...
      if (isEditing && editServiceId) {
        response = await axios.put(`https://bmytsqa7b3.ap-south-1.awsapprunner.com/api/customers/update-service/${editServiceId}`,  formDataToSend, {
        ...
        response = await axios.post('https://bmytsqa7b3.ap-south-1.awsapprunner.com/api/customers/add-service',  formDataToSend, {
          headers: { "Content-Type": "multipart/form-data"},
          maxContentLength: Infinity,
          maxBodyLength: Infinity,
        });
...
const toggleAvailability = async (serviceId: string, currentAvailable: boolean) => {
  ...
  try {
    await axios.put(
      `https://bmytsqa7b3.ap-south-1.awsapprunner.com/api/customers/${serviceId}/status`,
      null,
...
<img
  src={
    service.image && service.image.startsWith("http")
      ? service.image
      : service.image
      ? `https://bmytsqa7b3.ap-south-1.awsapprunner.com${service.image}`
      : "/placeholder.png"
  }
          ...
          {/* Scrollable Form Fields */}
            <div className="flex-1 overflow-y-auto pr-2">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                {/* File Upload (alternative to URL) */}
<div className="col-span-1 md:col-span-2">
  ...
  <button type="button" onClick={addPackage} className="text-blue-500 font-bold mt-1">+ Add Package</button>
            ...
            <div className="flex justify-end gap-3 mt-6 flex-shrink-0">
              <Button type="button" variant="secondary" onClick={() => {
                ...
                setEditServiceId(null);
                setFormData({ ...initialEmptyFormData });
                setServiceImageFile(null);   // 🔹 reset file upload
              }}>
                Cancel
              </Button>
              <Button type="submit">
  ...
  src={viewService?.image && viewService.image.startsWith("http")
    ? viewService.image
    : viewService?.image
    ? `https://bmytsqa7b3.ap-south-1.awsapprunner.com${viewService.image}`
    : "/placeholder.png"}
