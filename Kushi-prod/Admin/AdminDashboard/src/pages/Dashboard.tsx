...
export function Dashboard() {
  ...
  useEffect(() => {
    const fetchRecentBookings = async () => {
      try {
        const res = await axios.get("https://bmytsqa7b3.ap-south-1.awsapprunner.com/api/admin/recent-bookings");
 ...
 // Fetch all bookings, filter for today's and recent
    axios.get("https://bmytsqa7b3.ap-south-1.awsapprunner.com/api/admin/all-bookings")
      .then(res => {
  ...
  useEffect(() => {
    const fetchServiceReportCsv = async () => {
      try {
        const res = await axios.get("https://bmytsqa7b3.ap-south-1.awsapprunner.com/api/admin/service-report/csv", {
          responseType: "blob",
        });
  ...
  //recent activity
  useEffect(() => {
  setRecentActivities(getTodayActivities());
}, []);
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 md:gap-6">
        ...
        <Card className="shadow-lg border-0 overflow-hidden">
          ...
          <CardContent className="p-3 md:p-4">
            {upcomingBookings.length === 0 ? (
              ...
              <div className="space-y-3">
                {upcomingBookings.map((booking) => (
                  <div
                     key={booking.booking_id} // ✅ Matches backend field
                    className="flex items-center space-x-3 p-3 rounded-xl bg-gradient-to-r from-gray-50 to-gray-100 dark:from-gray-800 dark:to-gray-700 border border-gray-200 dark:border-gray-600"
                  >
            ...
            {`${booking.address_line_1 || ''}, ${booking.city || ''} ${booking.zip_code || ''}`}
          </span>
                      </div>
                    </div>
                    <div className="text-right flex-shrink-0">
                      ...
                      <Badge
                        variant={booking.bookinStatus === 'confirmed' ? 'success' : 'warning'}
                        className="text-xs"
                      >
                        {booking.status}
                      </Badge>
            ...
            {recentActivities.length === 0 ? (
              ...
              <div className="space-y-3">
                {recentActivities.map((activity, index) => (
                  <div
                    key={index}
                    className="flex items-start space-x-3 p-3 rounded-xl bg-gradient-to-r from-gray-50 to-gray-100 dark:from-gray-800 dark:to-gray-700 border border-gray-200 dark:border-gray-600"
                  >
                    <div
