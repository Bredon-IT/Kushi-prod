...
export function Settings() {
 ...
 const handleAddUser = async (e: React.FormEvent) => {
  ...
  try {
    await axios.post(Global_API_BASE + "/api/login/add-user",
...
const handleDeleteUser = async (adminId: number) => {
  ...
  try {
    await axios.delete(Global_API_BASE + `/api/login/delete-user/${adminId}`, {
      params: { adminId: admin?.adminId } // current logged-in admin
    });
...
const handlePasswordChange = () => {
  ...
  useEffect(() => {
    ...
    if (email) {
      axios
        .get<Admin>(Global_API_BASE + `/api/login/me/${email}`)
        .then((res) => {
          setAdmin(res.data);
